...
var require_lib3 = __commonJS({
  "node_modules/.pnpm/node-fetch@2.6.8/node_modules/node-fetch/lib/index.js"(exports2, module2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
...
async function auditDependencies(type, options) {
  return DEPENDENCY_AUDITORS[type](options);
}

// src/vulnerabilities/helpers.ts
var SEVERITY_LEVELS = ["info", "low", "moderate", "high", "critical"];
function isSeverityLevel(arg) {
  return SEVERITY_LEVELS.includes(arg);
}
function countVulnerabilities(report, level = "low") {
  return SEVERITY_LEVELS.slice(SEVERITY_LEVELS.indexOf(level)).reduce((total, level2) => {
    return total + (report.vulnerabilities[level2] ?? 0);
  }, 0);
}
function hasVulnerabilities(report, level = "low") {
  return countVulnerabilities(report, level) > 0;
}

// src/vulnerabilities/render.ts
function noVulnerabilities(packageName) {
  return `
  \u2705 No vulnerabilities found in **${packageName}**.
`;
}
function lowRenderer(val) {
  return `<tr>
  <th align="left">\u{1F535} Low</th>
  <td>${val}</td>
</tr>`;
}
function infoRenderer(val) {
  return `<tr>
  <th align="left">\u26AA Info</th>
  <td>${val}</td>
</tr>`;
}
function moderateRenderer(val) {
  return `<tr>
  <th align="left">\u{1F7E1} Moderate</th>
  <td>${val}</td>
</tr>`;
}
function highRenderer(val) {
  return `<tr>
  <th align="left">\u{1F7E0} High</th>
  <td>${val}</td>
</tr>`;
}
function criticalRenderer(val) {
  return `<tr>
  <th align="left">\u{1F534} Critical</th>
  <td>${val}</td>
</tr>`;
}
var VULNERABILITY_RENDERERS = {
  low: lowRenderer,
  info: infoRenderer,
...
function someVulnerabilities(packageName, vulnerabilities) {
  return `
  ## Vulnerabilities
  ...
  <table>
    <tbody>
    ${Object.entries(vulnerabilities).map(([k, v]) => VULNERABILITY_RENDERERS[k](v)).join("\n")}
    </tbody>
  </table>`;
}
...
async function run() {
  ...
  const name = core.getInput("package-name") || import_github.context.repo.repo;
